    ...
    <>
      ...
      <main className="mx-auto w-full max-w-[1080px] bg-white px-4 pb-12 sm:px-6 lg:px-8">
        <article className="blog-detail-article w-full">
          ...
          <div className="blog-table-wrap mt-4 overflow-x-auto"><table className="blog-data-table w-full min-w-[900px] border-collapse font-manrope text-[12px]"><thead><tr><th>Walker Name</th><th>Brand</th><th>Price</th><th>Rating</th><th>key features</th><th className="p-3" /></tr></thead><tbody>{tableRows.map(([name, brand, price, rating, features, href]) => <tr><td>{name}</td><td>{brand}</td><td>{price}</td><td>{rating}</td><td>{features}</td><td><a className="font-black text-[#0b61a4]" href={href}>Buy Now</a></td></tr>)}</tbody></table></div>
          <SectionHeading id="section-2">Top 5 Upright Walkers for Seniors</SectionHeading>
          {products.map((product) => <section className="mt-7" key={product.title}><h3 className="text-[15px] font-black text-black">{product.title}</h3><p className="mt-3 font-manrope text-[12px] font-black text-black">Rating: ★★★★★</p><p className="mt-1 font-manrope text-[12px] font-black text-black">Price: {product.price}</p><img className="mt-4 h-auto max-h-[420px] w-full object-contain" src={product.image} alt="" /><h4 className="mt-5 text-[14px] font-black text-black">Key Features</h4><ul className="mt-2 list-disc pl-5 font-manrope text-[12px] leading-6 text-[#1f2930]">{product.features.map((f) => <li key={f}>{f}</li>)}</ul><BlogProsCons className="mt-6" prosLabel={product.columns[0][0]} consLabel={product.columns[1][0]} pros={product.columns[0][1]} cons={product.columns[1][1]} /><h4 className="mt-7 text-[15px] font-black text-black">Verified Customer Reviews</h4><ReviewCarousel reviews={product.reviews} /></section>)}
